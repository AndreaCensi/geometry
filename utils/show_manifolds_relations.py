...
def embedding_type2color(t):
    t2color = {"lie": "blue", "user": "green", "derived": "darkgreen"}
    if not t in t2color:
...
def isomorphism_type2color(t):
    t2color = {"lie": "blue", "user": "red", "derived": "darkred"}
    if not t in t2color:
        print("Warning, unknown type %s" % t)
        color = "yellow"
    else:
...
def main():
    ...
    for m in all_manifolds:
        ...
        for m2, rel in m._isomorphisms.items():
            ...
            if id(m) < id(m2):
                g.add_edge(Edge(Node(str(m)), Node(str(m2)), color=color, undirected=True, dir="none"))
