...
from .manifold_relations import Embedding
from .manifold_relations import Isomorphism
from .manifold_relations import ManifoldRelations
...
class DifferentiableManifold:
    ...
    def isomorphism(A, B, A_to_B, B_to_A, itype="user", steps=None, desc=None):
        if A.dimension != B.dimension:
            msg = (
                "You are trying to define an isomorphism"
    ...
    def embedding(A, B, A_to_B, B_to_A, itype="user", steps=None, desc=None):
        if A.dimension > B.dimension:
            msg = (
                "You are trying to define an embedding"
    ...
    def project_to(self, m, my_point):
        if not self.can_represent(m):
            msg = "%s does not contain %s; %s" % (self, m, self.relations_descriptions())
            raise ValueError(msg)
    ...
    def convert_to(self, m, my_point):
        if not self.can_convert_to(m):
            msg = "%s cannot be converted to %s; %s" % (self, m, self.relations_descriptions())
            raise ValueError(msg)
